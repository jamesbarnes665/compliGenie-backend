...
namespace CompliGenie.Tests.AcceptanceTests
{
    public class Story_1_1_1_MultiTenantDatabaseTests : IDisposable
    {
        ...
        public Story_1_1_1_MultiTenantDatabaseTests()
        {
            ...
            var migrationScript = System.IO.File.ReadAllText(@".\database\migrations\001_multi_tenant_base.sql");
            // Note: In real test, we'd execute this against PostgreSQL
        }
        ...
        {
            // Create 1000 tenants
            var tenants = Enumerable.Range(1, 1000)
                .Select(i => new Tenant 
                { 
                    Id = Guid.NewGuid(), 
                    Name = $"Tenant{i}",
            ...
            var lastTenant = await _context.Tenants
                .Where(t => t.Name == "Tenant1000")
                .FirstAsync();
            sw2.Stop();

            // Performance should be similar (within 2x)
            Assert.True(sw2.ElapsedMilliseconds < sw1.ElapsedMilliseconds * 2,
                $"Query performance degraded: First={sw1.ElapsedMilliseconds}ms, Last={sw2.ElapsedMilliseconds}ms");
        }

        [Fact]
        public void AC4_Migration_Script_Valid()
        {
            // Verify migration script exists and is valid SQL
            var migrationPath = @".\database\migrations\001_multi_tenant_base.sql";
            Assert.True(System.IO.File.Exists(migrationPath), "Migration script not found");
            
            var script = System.IO.File.ReadAllText(migrationPath);
            Assert.Contains("CREATE TABLE tenants", script);
            Assert.Contains("CREATE TABLE policies", script);
            Assert.Contains("CREATE TABLE webhook_events", script);
            Assert.Contains("CREATE TABLE financial_alerts", script);
        }
        ...
        public void AC5_Payment_Tables_Include_Isolation()
        {
            ...
            Assert.Contains("webhook_events", script);
            Assert.Contains("tenant_id UUID REFERENCES tenants(id)", script);
            Assert.Contains("ALTER TABLE webhook_events ENABLE ROW LEVEL SECURITY", script);
            
            // Verify financial_alerts has tenant isolation
            Assert.Contains("financial_alerts", script);
            Assert.Contains("tenant_id UUID REFERENCES tenants(id)", script);
        }
